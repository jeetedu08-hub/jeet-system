    ...
    try:
        ...
        for _, s_row in df_results.iterrows():
            ...
            with PdfPages(pdf_buffer) as pdf:
                ...
                # 🌟 자간을 강제로 넓혀 시원하게 배치
                txt_jeet = fig.text(0.31, 0.88, 'J E E T', fontsize=42, fontweight='bold', color=COLOR_RED, ha='right')
                txt_title = fig.text(0.33, 0.88, '수 학  능 력  분 석  리 포 트', fontsize=32, fontweight='bold', color=COLOR_NAVY, ha='left')
                
                # 정보란 사이 공간(여백) 확대
                info_text = f"학교 : {s_row.get('학교', '')}    |    학년 : {student_grade}    |    이름 : {student_name}    |    과정 : {selected_test}"
                txt_info = fig.text(0.5, 0.84, info_text, ha='center', fontsize=14, fontweight='bold', color='#222')
                ...
                txt_info.set_path_effects([path_effects.withStroke(linewidth=0.5, foreground='#222')])
                ...
                ax1.set_title("▶ 영 역 별  핵 심  역 량  지 표 (%)", pad=30, fontsize=14, fontweight='bold', color=COLOR_NAVY)
                ...
                ax2.set_ylim(0, max_val * 1.5); ax2.set_title("▶ 단 원 별  성 취 도", pad=15, fontsize=14, fontweight='bold', color=COLOR_NAVY)
                ...
                # 🌟 줄 길이를 54 -> 50으로 줄여 양옆 여백을 늘림
                wrapped_lines = [textwrap.fill(p, width=50) for p in diag_content.split('\n\n')]
                
                # 🌟 줄 간격(linespacing)을 1.8 -> 2.3으로 대폭 넓혀 가독성 향상
                fig.text(0.11, 0.41, "\n\n".join(wrapped_lines), fontsize=10.5, linespacing=2.3, va='top', ha='left', color='#333')
