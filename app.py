...
# 🌟 로고 추가 부분
# 로고와 제목을 한 줄에 나란히 넣기 위해 컬럼을 사용합니다.
col1, col2 = st.columns([8, 2]) # 제목과 로고의 비율을 조절할 수 있습니다. (8:2 비율)

with col1:
    st.title("📊 JEET 죽전캠퍼스 성적 통합 관리 시스템")

with col2:
    # logo.png 파일이 깃허브 저장소에 있어야 합니다.
    if os.path.exists("logo.png"):
        st.image("logo.png", use_column_width=False, width=150) # use_column_width=False로 설정해서 원래 크기를 유지합니다. width로 너비를 조절할 수 있습니다.
    else:
        st.warning("logo.png 파일을 찾을 수 없습니다. 깃허브에 업로드해 주세요.")
