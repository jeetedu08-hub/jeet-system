...
# 💡 [핵심 업데이트] 35번 등 실제 등록된 문항번호(q_cols)만 추려 엑셀 컬럼을 잘라내는 함수
def export_excel_styled(df, quarter_name, q_cols):
    ...
    # 3행: 헤더 결합 (메타 정보 + 실제 매핑된 문항만 순서대로 붙이기)
...
# 💡 [핵심 업데이트] 35번까지만 칼같이 잘라서 출력해 주는 분기별 엑셀 다운로드 탭
with tab4:
    ...
    if st.button("📊 해당 분기 재원생 엑셀 파일 생성하기", type="primary"):
        ...
        if filtered_df.empty:
            st.warning(f"⚠ 현재 선택된 과정 [{selected_test}]의 [{excel_quarter}] 분기에는 '재원생' 데이터가 존재하지 않습니다.")
        else:
            # 2. test_info 정밀 분석하여 실제 문항 번호(예: 1번~35번) 목록 생성
            df_info_filtered = df_info_all[df_info_all['시험명'] == selected_test]
            ...
            st.success(f"🎯 총 {len(filtered_df)}명의 재원생 데이터가 확인되었습니다. 등록된 실제 시험 문항({len(actual_q_cols)}문항)만 선별해 정리합니다.")
            
            # 3. 선별된 문항 목록을 넘겨주어 50번까지 불필요한 빈 컬럼 차단 및 생성
