...
# --- 3. 공통 그래프 그리기 함수 (안정화 레이아웃 전면 수정) ---
def draw_report_figure(fig, s_row, student_name, student_grade, selected_test, cat_ratio, avg_cat_ratio, unit_data, unit_avg_data, unit_order):
    # 1. 외곽 테두리 (안정적인 패딩 확보)
    border = plt.Rectangle((0.02, 0.02), 0.96, 0.96, fill=False, edgecolor=COLOR_RED, linewidth=4.0, transform=fig.transFigure, zorder=10)
    ...
    if os.path.exists(logo_file):
        ...
        logo_img_axes = fig.add_axes([0.76, 0.92, 0.16, 0.04], zorder=15)
        logo_img_axes.imshow(logo_img)
        logo_img_axes.axis('off')

    # 3. 상단 타이틀 구역 (Y축 고정 배치를 통해 겹침 절대 차단)
    txt_jeet = fig.text(0.33, 0.925, 'JEET', fontsize=36, fontweight='bold', color=COLOR_RED, ha='right')
    txt_title = fig.text(0.35, 0.925, '수학 능력 분석 리포트', fontsize=26, fontweight='bold', color=COLOR_NAVY, ha='left')
    ...
    txt_info = fig.text(0.5, 0.885, info_text, ha='center', fontsize=11, fontweight='bold', color='#222')

    txt_jeet.set_path_effects([path_effects.withStroke(linewidth=2, foreground='white')])
    txt_title.set_path_effects([path_effects.withStroke(linewidth=1.5, foreground='white')])

    # 4. 방사형 그래프 구역 고정 (Y축 범위: 0.54 ~ 0.79)
    ax1 = fig.add_axes([0.10, 0.54, 0.34, 0.25], polar=True)
    ...
    ax1.fill(angles, s_vals, color=COLOR_RED, alpha=0.1) 
    ...
    for i in range(len(labels)):
        angle = angles[i]
        label_text = labels[i]
        dist_tb = ax1_limit * 1.12  
        dist_lr = ax1_limit * 1.08  
    ...
    ax1.set_title("▶ 영역별 핵심 역량 지표 (%)", pad=20, fontsize=13, fontweight='bold', color=COLOR_NAVY)

    # 5. 단원별 성취도 바 그래프 구역 고정 (Y축 범위: 0.54 ~ 0.79)
    ax2 = fig.add_axes([0.55, 0.56, 0.35, 0.21]) 
    ...
    ax2.set_xticklabels([textwrap.fill(str(l), 6) for l in final_unit_data.index], fontsize=8, fontweight='bold')
    ax2.tick_params(axis='x', which='both', length=0) 
    ax2.set_ylim(0, ax2_limit)
    ax2.grid(axis='y', color=COLOR_GRID, linestyle='-', linewidth=0.5, zorder=0)
    ax2.set_title("▶ 단원별 성취도 (%)", pad=20, fontsize=13, fontweight='bold', color=COLOR_NAVY)
    
    for i in range(len(x_pos)):
        val = int(s_pct.iloc[i])
        pos = x_pos[i]
        y_p = val + 3
        t = ax2.text(pos, y_p, f"{val}%", ha='center', va='bottom', fontsize=8, fontweight='bold', color=COLOR_STUDENT)
        t.set_path_effects([path_effects.withStroke(linewidth=2, foreground='white')])

    # 6. 하단 심층 분석 박스 영역 조정 (Y축: 0.12 ~ 0.50 영역 확장 및 레이어 zorder 보정)
    rect_diag = plt.Rectangle((0.05, 0.12), 0.90, 0.38, fill=True, facecolor=COLOR_BG, edgecolor=COLOR_GRID, transform=fig.transFigure, zorder=1)
    fig.patches.append(rect_diag)
    
    t_p1 = fig.text(0.08, 0.47, "▶ ", fontsize=13, fontweight='bold', color=COLOR_NAVY, zorder=2)
    t_p2 = fig.text(0.10, 0.47, " JEET", fontsize=13, fontweight='bold', color=COLOR_RED, zorder=2)
    t_p3 = fig.text(0.155, 0.47, f" 중등 자사 센터 {student_name} 학생 심층 분석", fontsize=13, fontweight='bold', color=COLOR_NAVY, zorder=2)
    ...
    diag_combined = ""
    if c_best: diag_combined += f"특히 {', '.join([f'[{c}]' for c in c_best])} 영역에서 정교한 논리 구조와 높은 응용력을 보이며 압도적인 강점을 나타냅니다. 최고 난도 문항을 통해 사고의 확장을 이어가야 합니다. "
    if c_good: diag_combined += f"{', '.join([f'[{c}]' for c in c_good])} 영역 또한 안정적인 정답률로 견고한 기본기를 증명하였습니다. 심화 유형에 대한 반복 훈련이 병행되어야 합니다. "
    if c_weak: diag_combined += f"{', '.join([f'[{c}]' for c in c_weak])} 영역은 복합 개념을 문제에 투영하는 과정에서 다소 고전하는 모습이 보입니다. 개념 간의 유기적 연결성을 이해하는 보완 작업이 필요합니다. "
    if c_warn: diag_combined += f"무엇보다 {', '.join([f'[{c}]' for c in c_warn])} 영역은 단원 간 연계성이 높은 만큼, 기초 개념의 재정립과 필수 유형에 대한 집중 학습이 시급합니다. "
    ...
    if g_best: diag_combined += f"{', '.join([f'<{u}>' for u in g_best])} 단원은 기본 및 응용 단계를 넘어 심화 단계까지 소화하고 있어 '킬러 문항' 중심의 도전적 학습이 필요합니다. "
    if g_good: diag_combined += f"{', '.join([f'<{u}>' for u in g_good])} 단원은 필수 유형들을 해결하고 있습니다. 심화 문항 도전 횟수를 늘려 사고의 유연성을 기른다면 더 발전할 수 있습니다. "
    if g_weak: diag_combined += f"{', '.join([f'<{u}>' for u in g_weak])} 단원은 개념의 조각들은 인지하고 있으나 실전 적용에서 병목 현상이 관찰되므로 핵심 유형의 반복 숙달이 필요합니다. "
    if g_warn: diag_combined += f"{', '.join([f'<{u}>' for u in g_warn])} 단원은 기초 체력이 부족할 뿐, 맞춤형 관리가 병행된다면 반등할 수 있으니 따뜻한 격려 부탁드립니다. "
    ...
    if avg_score >= 80:
        sol_text = f"{student_name} 학생은 뛰어난 성취를 보여주고 있습니다. JEET CARE+를 통해 고난도 사고력 문제를 즐길 수 있는 환경을 만들고, JDM을 활용한 자기주도학습과 주말몰입수업으로 심화를 다지며 성장을 이끌겠습니다."
    elif avg_score >= 50:
        sol_text = f"배운 내용을 소화하는 과정이 순조롭습니다. JEET CARE+를 통해 응용/심화 문제를 연습하고, JDM 위주의 자기주도학습 습관을 기르며 주말몰입수업을 통해 한층 더 성취도를 끌어올리겠습니다."
    elif avg_score >= 20:
        sol_text = f"현재 성장을 위해 에너지를 모으는 시기입니다. {units_text} 단원의 부족한 부분을 JEET CARE+ 밀착 관리를 통해 메우고, JDM 자습 습관화 및 주말몰입수업을 연계하여 실력을 견고히 다지겠습니다."
    else:
        sol_text = f"{student_name} 학생이 {units_text} 단원의 기초를 편안하게 받아들일 수 있도록 JEET CARE+와 JDM을 연계해 눈높이 지도를 진행하고 밀착관리 시스템으로 작은 성취의 기쁨을 체감하도록 정성을 다하겠습니다."

    sections = [("[종합 진단]", diag_total), ("[영역별&단원별 분석]", diag_combined), ("[JEET 맞춤 솔루션]", sol_text)]

    # [수정 완벽 방어] 줄바꿈 폭을 54글자 수준으로 타이트하게 묶어 프레임 탈출 근본적 차단
    wrap_width = 54        
    main_fs = 8.5          
    sub_fs = 9.5            
    y_offset = 0.016       
    ...
    for subtitle, content in sections:
        stxt = fig.text(0.08, curr_y, subtitle, fontsize=sub_fs, fontweight='bold', color=COLOR_NAVY, zorder=2)
        wrapped_content = textwrap.fill(content, width=wrap_width)
        ctxt = fig.text(0.08, curr_y - y_offset, wrapped_content, fontsize=main_fs, linespacing=1.5, va='top', color='#333', zorder=2)
    ...
    for i, (name, addr) in enumerate(campuses):
        ...
        fig.text(pos_x[i], 0.07, name, ha='center', fontsize=9, fontweight='bold', color=COLOR_NAVY)
...
def generate_jeet_expert_report(target_name, selected_test):
    try:
        ...
        for _, s_row in df_results.iterrows():
            ...
            plt.close(fig)
            img_buffer.seek(0)
            return img_buffer.getvalue(), student_name
            
        if not student_found:
            return None, None
    except Exception as e:
        print(f"리포트 생성 치명적 오류: {e}")
        traceback.print_exc()
        return None, None


# --- 5. 스트림릿 메인 웹 UI 대시보드 구역 ---
st.set_page_config(page_title="JEET 성적 관리 시스템", layout="wide")
st.title(CAMPUS_CFG["title_text"])
st.info(f"🌐 현재 연동된 데이터베이스 서버: **{CURRENT_CAMPUS}**")

try:
    df_info_raw, df_results_raw = fetch_all_dataframes()
    
    if df_results_raw.empty or df_info_raw.empty:
        st.warning("📊 현재 데이터베이스에 입력된 성적 원장 데이터 또는 시험지 정보(배점/단원)가 없습니다.")
    else:
        test_list = sorted(df_results_raw['시험명'].unique().tolist())
        selected_test = st.selectbox("🎯 분석 및 추출할 시험을 선택하세요", test_list)
        
        filtered_df = df_results_raw[df_results_raw['시험명'] == selected_test].copy()
        df_info_filtered = df_info_raw[df_info_raw['시험명'] == selected_test].copy()
        
        excel_quarter = filtered_df['분기'].astype(str).str.strip().iloc[0] if '분기' in filtered_df.columns and not filtered_df.empty else "통합"
        ...
        # --- 기능 1: 개별 학생 분석 리포트 이미지 바로 보기 및 다운로드 ---
        st.write("---")
        st.subheader("👤 학생별 성분석 분석 리포트 (PNG 이미지 실시간 추출)")
        
        student_list = sorted(filtered_df['이름'].unique().tolist())
        target_student = st.selectbox("리포트를 뽑을 학생의 이름을 선택하세요", student_list)
        
        if st.button(f"✨ {target_student} 학생 리포트 생성 및 이미지 확인"):
            with st.spinner("최고 화질 리포트 그래픽 렌더링 중..."):
                img_bytes, s_name = generate_jeet_expert_report(target_student, selected_test)
                if img_bytes:
                    st.image(img_bytes, caption=f"✨ JEET {s_name} 학생 수학 능력 분석 리포트", use_container_width=True)
                    st.download_button(
                        label=f"💾 {s_name}_수학_능력_분석_리포트.png 다운로드",
                        data=img_bytes,
                        file_name=f"{s_name}_리포트.png",
                        mime="image/png"
                    )
                else:
                    st.error("리포트 이미지 생성에 실패했거나 학생 정보를 찾을 수 없습니다.")
