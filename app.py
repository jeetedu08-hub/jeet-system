...
def load_data():
    # 기존 코드와의 호환성을 위해 함수명 유지
    df_info, df_results = fetch_all_dataframes()
    return df_info, df_results


# --- 3. 공통 그래프 그리기 함수 (원장님 분석 로직 100% 동일) ---
    ...
    try:
        ...
        with PdfPages(pdf_buffer) as pdf:
            for _, s_row in df_results.iterrows():
                ...
                unit_data = analysis.groupby('단원').agg({'득점': 'sum', '배점': 'sum'}).reindex([u for u in unit_order if u in unit_order])
                fig = plt.figure(figsize=(8.27, 11.69))
                draw_report_figure(fig, s_row, student_name, s_row.get('학년',''), selected_test, cat_ratio, avg_cat_ratio, unit_data, unit_avg_data, unit_order)
...
def generate_batch_report(target_class, selected_test, selected_students=None):
    try:
        ...
        if selected_students:
            class_students = class_students[class_students['이름'].astype(str).str.strip().isin([str(s).strip() for s in selected_students])]
        pdf_buffer = io.BytesIO()
        with PdfPages(pdf_buffer) as pdf:
            for _, s_row in class_students.iterrows():
                ...
                draw_report_figure(fig, s_row, student_name, s_row.get('학년',''), selected_test, cat_ratio, avg_cat_ratio, unit_data, unit_avg_data, unit_order)
                pdf.savefig(fig); plt.close(fig)
        return True, pdf_buffer, "일괄 생성 완료"
    except Exception: return False, None, traceback.format_exc()
...
try:
    ...
    tab1, tab2, tab3 = st.tabs(["📝 성적 입력", "📑 개별 리포트", "🗂 일괄 리포트"])
    ...
    with tab1: # ✨ 수파베이스 저장 로직 적용
        with st.form("input_form", clear_on_submit=True):
            ...
            if st.form_submit_button("성적 저장하기", type="primary"):
    ...
    with tab3: # 일괄 출력
        classes = sorted([c for c in df_results_all['반'].unique().tolist() if c and c not in ['0','nan']])
        sel_class = st.selectbox("반 선택", classes)
        students = sorted(df_results_all[(df_results_all['시험명']==selected_test)&(df_results_all['반']==sel_class)]['이름'].tolist())
        sel_students = st.multiselect("출력 학생 선택", students, default=students)
        if st.button("일괄 PDF 생성"):
            success, buf, msg = generate_batch_report(sel_class, selected_test, sel_students)
            if success: st.download_button("📥 일괄 다운로드", buf.getvalue(), f"{sel_class}_리포트.pdf")
            else: st.error(msg)
