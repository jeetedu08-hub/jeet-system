...
# --- 2. ✨ Supabase 연동 및 캐시 설정 (구글 시트 대체) ---
@st.cache_resource
...
# --- 3. 공통 그래프 그리기 함수 (원장님 원본 로직 100% 보존) ---
...
def generate_jeet_expert_report(target_name, selected_test):
    try:
        ...
        return (True, pdf_buffer, "리포트 생성 완료!") if student_found else (False, None, "학생 없음")
...
# --- 5. Streamlit 웹 UI (탭 3 필터링 로직 포함 원장님 코드 100%) ---
...
try:
    ...
    with tab2:
        ...
        if st.button("개별 PDF 리포트 생성", type="primary"):
            ...
            if success: st.success(msg); st.download_button("📥 PDF 다운로드", buf.getvalue(), f"{target_student}_리포트.pdf", "application/pdf")
            else: st.error(msg)
            ...
            if class_list:
                ...
                if students_in_class:
                    ...
                    if st.button("반 전체/선택 일괄 PDF 생성", type="primary"):
                        ...
                        if success: st.success(msg); st.download_button("📥 일괄 PDF 다운로드", buf.getvalue(), f"{target_class}_선택_리포트.pdf", "application/pdf")
                        else: st.error(msg)
except Exception as e:
