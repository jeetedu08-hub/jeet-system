...
# --- 2. 구글 스프레드시트 연동 (인증 오류 수정 버전) ---
@st.cache_resource
def get_google_sheet():
    scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
    
    # 1. Streamlit Secrets (배포 서버용)
    if "GOOGLE_JSON" in st.secrets:
        creds_dict = json.loads(st.secrets["GOOGLE_JSON"])
        creds = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, scope)
    # 2. 로컬 테스트용 (내 컴퓨터에 파일이 있을 때)
    elif os.path.exists("secrets.json"):
        creds = ServiceAccountCredentials.from_json_keyfile_name("secrets.json", scope)
    else:
        st.error("❌ 구글 인증 정보(Secrets)를 찾을 수 없습니다.")
        st.stop()
    ...
    try:
        ...
        for _, s_row in df_results.iterrows():
            ...
            with PdfPages(pdf_buffer) as pdf:
                ...
                border = plt.Rectangle((0.015, 0.015), 0.97, 0.97, fill=False, edgecolor=COLOR_RED, linewidth=5, transform=fig.transFigure)
                fig.patches.append(border)
                
                # 헤더 (JEET 2학년 1학기 분석 리포트)
                ...
                for i, row in enumerate(unit_data.itertuples()):
                    ax2.text(i, 0.3, f"{int(row.득점)}({int(row.배점)})", ha='center', fontsize=9, fontweight='bold')
                ...
                eval_tier = "최상위권의 성취도" if avg_val >= 90 else "보통의 성취도"
                worst_cat = (cat_ratio - avg_cat_ratio).idxmin()
                
                diag_text = (
                    f"1. 종합 진단: {target_name} 학생은 전체 평균({total_avg_val}%) 대비 {abs(diff)}%p {'높은' if diff >= 0 else '낮은'} {avg_val}%의 성적을 기록 중입니다.\n\n"
                    f"2. 강약점 분석: '{cat_ratio.idxmax()}' 역량은 우수하나, '{worst_cat}' 역량은 보완이 필요합니다.\n\n"
                    f"3. JEET 맞춤 솔루션: 취약한 '{worst_cat}' 영역의 개념 정립과 오답 노트를 통한 반복 학습을 권장합니다."
                )
